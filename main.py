...
async def main():
    ...
    todo3 = {
        ...
        2: ["add:fidgetcube"], # at Iter 2 add a new block with this data
        3: ["add:figetstick"]  # At iter 3, add a new block
    }

    # For the third node added later on. To demonstrate that the other nodes have learnt of it and can update themselves
    todo1 = {
        4: ["add:fidgetdoughnut"], #  at iter 4, add a new block
    ...
    asyncio.ensure_future(asyncio.gather(poll_forever("127.0.0.3", peer_list, todo3), poll_forever("127.0.0.2", peer_list, dict())))
